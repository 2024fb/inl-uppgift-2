...
function taskOne() {
  ...
  if (blue = !blue) {
    document.querySelector("#answer-one").style.backgroundColor = "white";

  } else {
    document.querySelector("#answer-one").style.backgroundColor = "blue";
   }     
}
...
function multiplier(valueOne, valueTwo) {
  ...
  if (isNaN (valueOne) || isNaN (valueTwo) || isNaN (valueOne && valueTwo)) {
...
function taskSix() {
  ...
  document.getElementById("answer-six").innerHTML = "<b>Över 30:</b>" + overThirty + 
   "<br> <b>Under 30:</b>" + underThirty + "<br> <b>Gift:</b>" + married + "<br> <b>Ogift:</b>" + notMarried; 
}
...
function taskSeven() {
  ...
  let addMeSomeLineBreaksNow = "";

  for (let val of addMeSomeLineBreaks) {
    switch(val) {
      case ",":
        addMeSomeLineBreaksNow += ", <br>";
        break;
    default:
      addMeSomeLineBreaksNow += val;    
    }
  }
  document.querySelector("#answer-seven").innerHTML = addMeSomeLineBreaksNow; 
}
...
const idCard = document.getElementById("card-eight");
...
function taskNine() {
  ...
  let allClasses = document.getElementsByClassName("answer-container");
  for (let i = 0; i < allClasses.length; i++) {
   
  if (date.getHours(hour + ":" + min) > 17.00) {
    allClasses[i].style.backgroundColor = "blue";
  } else {
    allClasses[i].style.backgroundColor = "red";
    }
  } 
}
...
function calculator(valueOne, valueTwo, operator) {
  ...
  const formatedOperator = operator.toLowerCase();
 
  if (typeof valueOne !== 'number' || typeof valueOne !== 'number') {
   alert("Något är fel");
  } else {
    switch(formatedOperator){

      case "add":
        alert("Resultat: " + (valueOne + valueTwo));
        break;
